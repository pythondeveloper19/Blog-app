class PostsController < ApplicationController
  before_filter :authorize_blogger!, :except => [:show]
  include PostsHelper
  ...
  def all_post
    @posts = Post.paginate(:page => params[:page]).order("created_at DESC") rescue []
    respond_to do |format|
  ...
  def refresh_comments
    @post=Post.where(:id =>  params[:post_id]).first rescue nil
    @post_comments = @post.comments.order("created_at ASC").last(4 * params[:no_click].to_i) rescue []
  end
